...
- [x] Integration test covers skill render+symlink, agent render, drift detection, remove/prune.
- [x] e2e test covers `install`/`add`/`remove` for `codex` and `claude,codex` targets.
- [x] Regression test: a manifest with no `targets` field installs identically to pre-change behaviour.
- [x] Codex-target code coverage is at or above the project threshold (>= 80%).
- [x] Full suite green: `pytest`, `mypy --strict`, `ruff`, `black`.
