...
│   │   └── settings.fragment.json  # чистый Claude Code config
...
> Опциональные опинионированные бандлы делаются как мета-домены: создай
> `catalog/my-bundle/domain.json` со списком `depends: [...]` —
> `ai-dotfiles add @my-bundle` подтянет всю цепочку транзитивно.
...
ai-dotfiles update                Обновить системные файлы в хранилище (сейчас — встроенный скилл ai-dotfiles)
...
ai-dotfiles list --available      Всё в catalog/
...
# --- Бандлы делаются как мета-домены: создай catalog/<bundle>/domain.json
# со списком "depends": [...] и используй ai-dotfiles add @bundle.

# --- Вендоринг ---

ai-dotfiles vendor list                           Список vendor-плагинов + статус зависимостей (с URL для отсутствующих)
ai-dotfiles vendor installed                      Вендорённые элементы каталога
ai-dotfiles vendor search <query>                 Агрегированный поиск по всем vendor-плагинам с группировкой
ai-dotfiles vendor remove <name>                  Удалить вендорённый элемент
ai-dotfiles vendor <plugin> install <source>      Поставить элемент из источника
ai-dotfiles vendor <plugin> list <source>         Перечислить, что источник отдаёт
ai-dotfiles vendor <plugin> search <query>        Поиск в каталоге плагина (если поддерживается)
ai-dotfiles vendor <plugin> refresh               Обновить кэш плагина (если поддерживается)
ai-dotfiles vendor <plugin> deps check            Проверить runtime-зависимости плагина
```
...
# → Для каждого @domain читает domain.json, разворачивает depends транзитивно
# → Для @domain: симлинки skills/, agents/, rules/, hooks/
# → Для skill:/agent:/rule:: симлинки standalone элементов
# → Собирает .claude/settings.json из settings.fragment.json (в топологическом порядке)
# → Пропускает domain.json, README.md, *.fragment.json; chmod +x на .sh
...
### Бандлы как мета-домены

```bash
ai-dotfiles domain create backend
# → catalog/backend/{skills/, agents/, rules/, hooks/, domain.json}

# Затем редактируй catalog/backend/domain.json:
#   "depends": ["@python", "@gitflow", "skill:code-review"]

ai-dotfiles add @backend
# → Транзитивно подтягивает @python, @gitflow, skill:code-review
# → Записывает их + сам @backend в ai-dotfiles.json (топологический порядок)
```

### vendor

CLI — это группа: у каждого плагина свой поддерево с `install`,
`list`, `deps check`; `search`/`refresh` появляются там, где плагин
их реализует.
...
# → Создаёт .source (vendor, origin, tool, fetched, license)
...
# → catalog/python/{skills/, agents/, rules/, hooks/, domain.json}
# → domain.json — заготовка с name + description; settings.fragment.json
#   и mcp.fragment.json создаются вручную по мере необходимости
...
# → Предупреждает если используется в global.json
```
...
# → Предупреждает если используется в ai-dotfiles.json или global.json
```
...
cd ~/projects/my-api
ai-dotfiles init
# Опинионированный бандл — один add, остальное транзитивно через depends:
ai-dotfiles add @backend
# или вручную, по штучно:
ai-dotfiles add @python skill:code-review agent:researcher
...
│   │   │   ├── SKILL.md                # Полный YAML frontmatter (+ опц. depends:)
│   │   │   ├── scripts/check.sh
...
│   │   └── settings.fragment.json      # Хук-декларации (чистый Claude config)
│   │
│   ├── skills/                         # Standalone скиллы
...
Канонический манифест метаданных домена. Все поля опциональны.
...
`depends` поддерживает спецификаторы манифеста (`@domain`, `skill:name`, `agent:name`, `rule:name`); CLI разворачивает их транзитивно при `add` и блокирует `remove`, если кто-то ещё зависит от цели. `requires.npm` — host-пакеты, которые проверяются по `package.json` проекта.

### settings.fragment.json

Чистый Claude Code config — никаких meta-ключей (они живут в `domain.json`).
...
# Маркетплейсы с локальным кэшем
ai-dotfiles vendor buildwithclaude search typescript
ai-dotfiles vendor buildwithclaude install <name>

# CLI-обёртки
ai-dotfiles vendor skills_sh install vercel-labs/skills --select frontend-design
ai-dotfiles vendor paks install kubernetes-deploy

# Ручной импорт тоже допустим — просто скопировать в catalog/ и
# дописать .source вручную.
```

`.source` (пишется автоматически, формат единый для всех плагинов):

```
vendor: github
origin: github:user/repo/skills/frontend-design
...
- **Бандлы как мета-домены**: создай `catalog/<bundle>/domain.json` с `depends: [...]` — `ai-dotfiles add @bundle` подтянет всё транзитивно
